...
import type { EmailStudioDashboardProps, EmailStudioVisualVariant } from "./email-studio-dashboard";
...
const dashboardConcepts: { key: EmailStudioVisualVariant; label: string; tagline: string }[] = [
  {
    key: "command-center",
    label: "Command center",
    tagline: "A familiar control room layout with live preview and performance tables.",
  },
  {
    key: "aurora",
    label: "Aurora gradients",
    tagline: "A high-gloss concept with glassmorphism, gradients, and campaign timelines.",
  },
  {
    key: "split",
    label: "Split console",
    tagline: "A dual-column analyst view pairing KPIs with the builder workspace.",
  },
  {
    key: "minimal",
    label: "Minimal digest",
    tagline: "A stripped-back index focused on quick scanning and status badges.",
  },
];
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-12 px-4 py-12 sm:px-6 lg:px-8">
      <header className="space-y-3">
        ...
        <p className="text-sm text-muted-foreground sm:text-base">
          Build one-off announcements, recurring touchpoints, and condition-based journeys — all while keeping performance in
          view.
        </p>
      </header>

      <div className="space-y-16">
        {dashboardConcepts.map((concept, index) => (
          <section key={concept.key} className="space-y-6">
            <div className="space-y-2">
              <p className="text-xs font-semibold uppercase tracking-[0.35em] text-muted-foreground">
                Concept {index + 1}
              </p>
              <h2 className="text-2xl font-semibold sm:text-3xl">{concept.label}</h2>
              <p className="text-sm text-muted-foreground sm:text-base">{concept.tagline}</p>
            </div>
